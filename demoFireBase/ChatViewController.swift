...
class ChatViewController: JSQMessagesViewController {
    ...
    private var messageRef: DatabaseReference = Database.database().reference().child("message")
    //get my avata
    ...
    var storageImageRef: StorageReference = Storage.storage().reference().child("imageFolder")
    ...
    func setImageUserLoginURL(_ url: String, forPhotoMessageWithKey key: String) {
        ...
        let value = ["photoURL":url]
        itemRefcurrentUserLogin.updateChildValues(value)
    }
    
    private var photoMessageMap = [String: JSQPhotoMediaItem]()
    private let loginID = Auth.auth().currentUser?.uid
    ...
    private func observeMessages() {
        ...
        updatedMessageRefHandle = messageQuery.observe(.childChanged, with: { (snapshot) in
            ...
            if let photoURL = messageData["photoURL"] as? String! { // 2
                ...
                if let mediaItemA = mediaItem {
                    self.fetchImageDataAtURL(photoURL, forMediaItem: mediaItemA, clearsPhotoMessageMapOnSuccessForKey: key) // 4
                    print("success")
                }
...
extension ChatViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    func imagePickerController(_ picker: UIImagePickerController,
        ...
        if let originalImg = info["UIImagePickerControllerOriginalImage"] {
            ...
            let uploadImage = selectedImg?.resizedEX(withPercentage: 0.1)
            let dataImage = UIImagePNGRepresentation(uploadImage!)
            let keyLogin = sendPhotoMessage()
            if let dataImage = dataImage {
                self.storageImageRef.child("/ \(result)").putData(dataImage, metadata: nil, completion: { (metadata, errordata) in
                    //save image into firebase db
                    if let imageURL = metadata?.downloadURL()?.absoluteString, let keyLogin = keyLogin {
                        self.setImageUserLoginURL(imageURL, forPhotoMessageWithKey: keyLogin)
                    }
